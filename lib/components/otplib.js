...
otplib.prototype.hotp = function hotp(secret, counter) {
  ...
  var _secret = this.helpers.stringToHex(secret),
      ...
      _counter = this.helpers.intToHex(counter),
      _counter = this.helpers.pad(_counter, 16),

      // jssha library
      // crypto = new cryptoHmac(counter, 'HEX'),
      // hmac = crypto.getHMAC(secret, 'HEX', 'SHA-1', 'HEX'),

      // crypto library
      _crypto = cryptoHmac.createHmac('sha1', new Buffer(_secret, 'hex')),
...
/*
 *  Uttilities to management secrets
 */
...
otplib.prototype.details = {
    ...
    uri: function uri(user, host, secret) {
      ...
      _data = _data.replace('%host', _host);
    ...
    qrcode: function qrcode(uri) {
      ...
      var _link = 'https://chart.googleapis.com/chart?cht=qr&chs=150x150&choe=UTF-8&chld=M|0&chl=%uri';
      _link = _link.replace('%uri', uri);

...
export function parseDotenvxInput(raw: string): ParsedEntry[] {
  let json: unknown;
  try {
    json = JSON.parse(raw);
  } catch {
    throw new Error("Invalid JSON input from dotenvx");
  }

  if (typeof json !== "object" || json === null || Array.isArray(json)) {
    throw new Error("Invalid input: expected JSON object from dotenvx get --all");
  }

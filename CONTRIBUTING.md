...
3. **Publish**: Trigger the **Publish to npm** workflow via `workflow_dispatch` (Actions tab) to build and publish packages. `pnpm run audit` (production dependencies) blocks every CI run and the publish dry run; tick `skip-audit` on the publish dispatch only for an explicitly accepted risk

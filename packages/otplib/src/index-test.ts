...
export function createOtplibTests(ctx: OtplibTestContext): void {
  ...
  describe("otplib", () => {
    ...
    // Regression: generateSync used to drop the `hooks` option while generate,
    // verify and verifySync all passed it through, so a custom encodeToken
    // (e.g. Steam Guard-style) was silently ignored by sync generation and the
    // resulting token would never verify.
    describe("hooks", () => {
      const hooks = { encodeToken: steamEncodeToken, validateToken: steamValidateToken };

      it("should produce the same TOTP token from generateSync and generate with custom hooks", async () => {
        const options = {
          secret: TEST_SECRET,
          epoch: 1234567890,
          digits: 5,
        ...
        const syncToken = generateSync(options);
        const asyncToken = await generate(options);

        expect(syncToken).toBe(asyncToken);
        for (const ch of syncToken) {
          expect("23456789BCDFGHJKMNPQRTVWXY").toContain(ch);
        }
      });

      it("should produce the same HOTP token from generateSync and generate with custom hooks", async () => {
        const options = {
          secret: TEST_SECRET,
          strategy: "hotp" as const,
          counter: 0,
          digits: 5,
          hooks,
        };
      ...
      it("should verify a TOTP token generated by generateSync with custom hooks", () => {
        const options = {
          secret: TEST_SECRET,
          epoch: 1234567890,
          digits: 5,
        ...
        const token = generateSync(options);
        const result = verifySync({ ...options, token });

        expect(result.valid).toBe(true);
      });

      it("should verify a HOTP token generated by generateSync with custom hooks", () => {
        const options = {
          secret: TEST_SECRET,
          strategy: "hotp" as const,
          counter: 0,

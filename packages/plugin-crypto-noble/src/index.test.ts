...
import { NobleCryptoPlugin } from "./index";
...
describe("NobleCryptoPlugin", () => {
  ...
  describe("hmac", () => {
    ...
    it("should produce deterministic HMAC results", async () => {
      const jsPlugin = new NobleCryptoPlugin();
      const secret = stringToBytes("abcde");
      const message = stringToBytes("fghij");

      const result1 = await jsPlugin.hmac("sha1", secret, message);
      const result2 = await jsPlugin.hmac("sha1", secret, message);
